...
export async function analyze(req, res) {
    ...
    const tabConfig = AnalyzeInputSchema[tab];
    ...
    const userPrompt = buildUserPrompt(tab, relationship, tone, content_type, subject_name ?? null, tab === 'pattern' ? { messages } : { message });
    try {
        const t0 = Date.now();
